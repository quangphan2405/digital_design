...
ARCHITECTURE struct OF c3_t2_top_level IS
   ...
   SIGNAL alien_x_coord1 : std_logic_vector(7 DOWNTO 0);
   SIGNAL alien_y_coord1 : std_logic_vector(7 DOWNTO 0);
...
BEGIN
   ...
   u_4combo_proc: PROCESS(gun_x_coord, alien_x_coord1, module_select)
   BEGIN
      CASE module_select IS
      WHEN "00" => dout2 <= gun_x_coord;
      WHEN "01" => dout2 <= alien_x_coord1;
      WHEN OTHERS => dout2 <= (OTHERS => 'X');
      END CASE;
   END PROCESS u_4combo_proc;

   -- ModuleWare code(v1.12) for instance 'U_5' of 'mux'
   u_5combo_proc: PROCESS(gun_y_coord, alien_y_coord1, module_select)
   BEGIN
      CASE module_select IS
      WHEN "00" => dout1 <= gun_y_coord;
      WHEN "01" => dout1 <= alien_y_coord1;
      WHEN OTHERS => dout1 <= (OTHERS => 'X');
      END CASE;
   END PROCESS u_5combo_proc;

   -- ModuleWare code(v1.12) for instance 'U_6' of 'mux'
   u_6combo_proc: PROCESS(gun_color, alien_color, module_select)
   BEGIN
      CASE module_select IS
      WHEN "00" => dout <= gun_color;
      WHEN "01" => dout <= alien_color;
   ...
   U_3 : c2_t7_x_coord
      PORT MAP (
         ...
         x_coord        => alien_x_coord1,
         y_coord        => alien_y_coord1
      );
